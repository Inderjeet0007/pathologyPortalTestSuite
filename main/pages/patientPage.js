...
const addPatientWithoutEquipment = (async (driver, name, email, phone, height, weight, age, systolic, diastolic, mode) => {
    try{
        ...
        console.log(`-> Finding and sending data to element: patientHeight`);
        let patientHeight = await driver.findElement(By.xpath("//input[@name='height']"));
        await sendKeysHandler(driver, patientHeight, height);
        console.log(`-> Finding and sending data to element: patientWeight`);
        let patientWeight = await driver.findElement(By.xpath("//input[@name='weight']"));
        await sendKeysHandler(driver, patientWeight, weight);
        console.log(`-> Finding and sending data to element: patientAge`);
        let patientAge = await driver.findElement(By.xpath("//input[@name='age']"));
        await sendKeysHandler(driver, patientAge, age);
        console.log(`-> Finding and sending data to element: patientSystolic`);
        let patientSystolic = await driver.findElement(By.xpath("//input[@name='systolic']"));
        await sendKeysHandler(driver, patientSystolic, systolic);
        console.log(`-> Finding and sending data to element: patientDiastolic`);
        let patientDiastolic = await driver.findElement(By.xpath("//input[@name='diastolic']"));
        await sendKeysHandler(driver, patientDiastolic, diastolic);
